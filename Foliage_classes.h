...
struct UFoliageInstancedStaticMeshComponent : UHierarchicalInstancedStaticMeshComponent {
	...
	bool bEnableDiscardOnLoad; // 0x8b8(0x01)
...
struct UFoliageStatistics : UBlueprintFunctionLibrary {
	...
	int32_t FoliageOverlappingSphereCount(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FVector centerPosition, float Radius); // Function Foliage.FoliageStatistics.FoliageOverlappingSphereCount // (Final|Native|Static|Public|HasDefaults|BlueprintCallable) // @ game+0x3bd8900
	void FoliageOverlappingBoxTransforms(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FBox Box, struct TArray<struct FTransform>& OutTransforms); // Function Foliage.FoliageStatistics.FoliageOverlappingBoxTransforms // (Final|Native|Static|Public|HasOutParms|HasDefaults|BlueprintCallable) // @ game+0x3bd8770
	int32_t FoliageOverlappingBoxCount(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FBox Box); // Function Foliage.FoliageStatistics.FoliageOverlappingBoxCount // (Final|Native|Static|Public|HasDefaults|BlueprintCallable) // @ game+0x3bd8610
};
...
struct UFoliageType_InstancedStaticMesh : UFoliageType {
	...
	struct UFoliageInstancedStaticMeshComponent* ComponentClass; // 0x4d0(0x08)
};
...
struct AInteractiveFoliageActor : AStaticMeshActor {
	...
	float FoliageDamageImpulseScale; // 0x2f0(0x04)
	float FoliageTouchImpulseScale; // 0x2f4(0x04)
	float FoliageStiffness; // 0x2f8(0x04)
	float FoliageStiffnessQuadratic; // 0x2fc(0x04)
	float FoliageDamping; // 0x300(0x04)
	float MaxDamageImpulse; // 0x304(0x04)
	float MaxTouchImpulse; // 0x308(0x04)
	float MaxForce; // 0x30c(0x04)
	float Mass; // 0x310(0x04)
	char pad_314[0x4]; // 0x314(0x04)

	void CapsuleTouched(struct UPrimitiveComponent* OverlappedComp, struct AActor* Other, struct UPrimitiveComponent* OtherComp, int32_t OtherBodyIndex, bool bFromSweep, struct FHitResult& OverlapInfo); // Function Foliage.InteractiveFoliageActor.CapsuleTouched // (Final|Native|Protected|HasOutParms) // @ game+0x3bd83a0
};
...
struct AProceduralFoliageBlockingVolume : AVolume {
	struct AProceduralFoliageVolume* ProceduralFoliageVolume; // 0x2b0(0x08)
...
struct UProceduralFoliageSpawner : UObject {
	...
	void Simulate(int32_t NumSteps); // Function Foliage.ProceduralFoliageSpawner.Simulate // (Final|Native|Public|BlueprintCallable) // @ game+0x3bd8a80
};
...
struct AProceduralFoliageVolume : AVolume {
	struct UProceduralFoliageComponent* ProceduralComponent; // 0x2b0(0x08)
};
